...
export default function NoteCard({note, setNotes}) {
  ...
  return (
    <>
    <Link to={`/note/${note._id}`} className=" p-4 card bg-base-100 border-t-4 h-60 w-90 hover:shadow-xl transition-all duration-200 border-solid border-[#00FF9D]" >
      <div className="card-body bg-base-500 shadow-2xl w-90">
        <h2 className="card-title text-md font-bold ">{note.title} </h2>
        <p className='text-base-content/80 text-md'>{note.content} </p>
